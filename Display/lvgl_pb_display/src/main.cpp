...
int main(void)
{
    ...
    // TODO: color
    VerticalBar *tempBar = new VerticalBar(cont, 23, 0, 1, 4);
    ...
    NumericalValue *batAmp = new NumericalValue(cont, 1, 1, 3, 1);
    batAmp->init("Batt\nAmps");
    batAmp->draw();
    batAmp->setValue(43);
    ...
    Gauge *rpm = new Gauge(cont, 16, 0, 4, 4);
    ...
    NumericalValue *escTemp = new NumericalValue(cont, 1, 2, 3, 1);
    escTemp->init("ESC\n°C");
    escTemp->draw();
    escTemp->setValue(25.9);

    NumericalValue *motTemp = new NumericalValue(cont, 1, 3, 3, 1);
    motTemp->init("Motor\n°C");
    motTemp->draw();
    motTemp->setValue(60.9);

    NumericalValue *mileage = new NumericalValue(cont, 20, 0, 3, 1);
    mileage->init("kWh/\n100km");
    mileage->draw();
    mileage->setValue(5);

    NumericalValue *distance = new NumericalValue(cont, 20, 1, 3, 1);
    distance->init("Travel.\nkm", "%5.2f");
    distance->draw();
    distance->setValue(13.37);

    NumericalValue *range = new NumericalValue(cont, 20, 2, 3, 1);
    range->init("Est.\nkm", "%5.2f");
    range->draw();
    range->setValue(42.23);

    NumericalValue *topSpd = new NumericalValue(cont, 20, 3, 3, 1);
    topSpd->init("Max\nkm/h");
    topSpd->draw();
    topSpd->setValue(66);
